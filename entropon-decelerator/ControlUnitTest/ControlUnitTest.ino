...
void setup() {
    ...
    for (uint8_t i = 0; i < 4; i++) {
        Serial.print(F("Clearing display "));
        Serial.print(i + 1);
        Serial.print(F("..."));
        clearDisplay(i);
    ...
    // Display 111111, 222222, 333333, 444444
    for (uint8_t i = 0; i < 4; i++) {
        ...
        displayNumber(i, i + 1);  // Display 1 shows 1s, Display 2 shows 2s, etc.
        
        Serial.println(F(" ✓"));
        delay(200);  // Small delay between displays for visual effect
    }
    
    Serial.println(F("\n================================="));
    Serial.println(F("Test Complete!"));
    Serial.println(F(""));
    Serial.println(F("You should see:"));
    Serial.println(F("  Display 1: 111111"));
    Serial.println(F("  Display 2: 222222"));
    Serial.println(F("  Display 3: 333333"));
    Serial.println(F("  Display 4: 444444"));
    Serial.println(F("\nIf any display is not showing"));
    Serial.println(F("correctly, check its wiring."));
    Serial.println(F("=================================\n"));
}
    ...
    if (millis() - lastChange > 3000) {
        ...
        switch(testMode) {
            ...
            case 2:
                ...
                for (uint8_t displayNum = 0; displayNum < 4; displayNum++) {
                    // Create 123456 pattern
                    uint8_t segments[6];
                    for (uint8_t i = 0; i < 6; i++) {
                        segments[i] = digitSegments[(i + 1) % 10];
                    }
                    
                    // Send to display with reversal and brightness
                    sendSegmentsToDisplay(displayNum, segments);
                }
                break;
